...
public class RockManager {
    ...
    public RockManager() {
        ...
        for( int i = 0; i < 1; i++ ){
            ...
            float newZ = (Math.random() < 0.5) ? 1.0f : -1.0f;

            Rock r = new Rock( 0f, 0f, 1f);
        ...
        while(itr.hasNext()){
            ...
            if(!currentCollision) {
                ...
                if (Math.abs((float) r.getX() - (float) s.getX()) < .1f &&
                        ...
                        Math.abs((float) r.getZ() - (float) s.getZ()) < .1f) {
                    s.explode();
                }
            }
        }
    ...
    public void drawAll(float[] VP){
        ...
        while(itr.hasNext()) {
            ...
            Matrix.translateM(T, 0, r.getX(), r.getY(), r.getZ() );

            Matrix.setIdentityM(R, 0);
            Matrix.setRotateM(R, 0, r.getDirection(), 0.0f, 1.0f, 1.0f);

            Matrix.setIdentityM(S, 0);
            Matrix.scaleM(S, 0, 0.05f, 0.05f, 0.05f);
    ...
    public synchronized void spawn() {
        ...
        Rock r = new Rock( newX, newY, newZ);
